...
mod cuda_bench {
    ...
    pub fn bench_crossattention(c: &mut Criterion) {
        ...
        for (name, dim, heads, head_dim, ctx_dim) in configs {
            ...
            let std_attn = CrossAttention::<BenchBackend>::new(dim, heads, head_dim, ctx_dim, &device);
            ...
            group.bench_with_input(
                ...
                |b, _| {
                    b.iter(|| {
                        let out = std_attn.forward(
                            black_box(query_std.clone()),
                            black_box(context_std.clone()),
                        );
                        let _ = out.into_data();
                    });
                },
            ...
            let cubecl_attn = CrossAttentionCubeCL::<CudaRuntime>::new(dim, heads, head_dim, ctx_dim, &device);
