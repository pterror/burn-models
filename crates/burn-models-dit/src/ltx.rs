...
//!
//! LTX-Video is Lightricks' fast video generation model that uses
//! an efficient DiT architecture with causal temporal attention.
//!
...
//!        ↓
//! [DiT Blocks with Causal Temporal Attention]
//!        ↓
...
//! - **Causal temporal**: Autoregressive-style temporal attention
...
impl LtxVideoConfig {
    ...
    pub fn init<B: Backend>(&self, device: &B::Device) -> (LtxVideo<B>, LtxVideoRuntime<B>) {
        ...
        // Timestep embedding
        let time_embed = LtxTimestepEmbed {
            ...
            linear2: LinearConfig::new(self.hidden_size, self.hidden_size)
                .with_bias(true)
                .init(device),
            freqs: ltx_timestep_freqs(self.time_embed_dim, device),
        };
...
/// Precompute sinusoidal timestep frequencies for LTX-Video.
#[rustfmt::skip]
pub fn ltx_timestep_freqs<B: Backend>(embed_dim: usize, device: &B::Device) -> Tensor<B, 1> {
...
/// Timestep embedding for LTX-Video
#[derive(Module, Debug)]
pub struct LtxTimestepEmbed<B: Backend> {
...
impl<B: Backend> LtxTimestepEmbed<B> {
...
impl LtxBlockConfig {
    ...
    fn init<B: Backend>(&self, device: &B::Device) -> LtxBlock<B> {
        ...
        LtxBlock {
            ...
            temporal_attn: LtxCausalAttention {
                to_qkv: LinearConfig::new(self.hidden_size, 3 * self.hidden_size)
                    .with_bias(true)
                    .init(device),
                to_out: LinearConfig::new(self.hidden_size, self.hidden_size)
...
/// LTX Spatial Attention with RoPE
#[derive(Module, Debug)]
pub struct LtxAttention<B: Backend> {
    pub to_qkv: Linear<B>,
    pub to_out: Linear<B>,
    #[module(skip)]
    pub num_heads: usize,
...
/// LTX Causal Temporal Attention with RoPE
#[derive(Module, Debug)]
pub struct LtxCausalAttention<B: Backend> {
    pub to_qkv: Linear<B>,
    pub to_out: Linear<B>,
    #[module(skip)]
    pub num_heads: usize,
...
/// LTX Cross-Attention to text
#[derive(Module, Debug)]
pub struct LtxCrossAttention<B: Backend> {
    pub to_q: Linear<B>,
    pub to_kv: Linear<B>,
    pub to_out: Linear<B>,
    #[module(skip)]
...
impl<B: Backend> LtxCrossAttention<B> {
...
/// LTX DiT Block with factorized attention
#[derive(Module, Debug)]
pub struct LtxBlock<B: Backend> {
    pub norm1: LayerNorm<B>,
    pub spatial_attn: LtxAttention<B>,
    pub norm2: LayerNorm<B>,
    pub temporal_attn: LtxCausalAttention<B>,
    pub norm3: LayerNorm<B>,
    pub cross_attn: LtxCrossAttention<B>,
...
impl<B: Backend> LtxBlock<B> {
    ...
    pub fn forward(
        ...
        // Causal temporal attention: [B, T*H*W, D] -> [B*H*W, T, D]
...
/// LTX Final Layer
#[derive(Module, Debug)]
pub struct LtxFinalLayer<B: Backend> {
    pub norm: LayerNorm<B>,
    pub proj: Linear<B>,
    pub modulation: Linear<B>,
}

impl<B: Backend> LtxFinalLayer<B> {
...
/// LTX-Video Model
#[derive(Module, Debug)]
pub struct LtxVideo<B: Backend> {
    pub video_embed: Linear<B>,
    pub text_embed: Linear<B>,
    pub time_embed: LtxTimestepEmbed<B>,
    pub blocks: Vec<LtxBlock<B>>,
    pub final_layer: LtxFinalLayer<B>,
    #[module(skip)]
    pub hidden_size: usize,
    #[module(skip)]
    pub patch_size: usize,
...
mod tests {
    ...
    #[test]
    fn test_ltx_config() {
        let config = LtxVideoConfig::base();
        assert_eq!(config.hidden_size, 2048);
        assert_eq!(config.num_blocks, 28);
    }

    #[test]
    fn test_ltx_timestep_embed() {
        let device = Default::default();
        let embed = LtxTimestepEmbed {
            linear1: LinearConfig::new(64, 256).with_bias(true).init(&device),
            linear2: LinearConfig::new(256, 256).with_bias(true).init(&device),
            freqs: ltx_timestep_freqs(64, &device),
        };
    ...
    #[test]
    fn test_ltx_spatial_attention() {
        let device = Default::default();
        let attn = LtxAttention {
    ...
    #[test]
    fn test_ltx_causal_attention() {
        let device = Default::default();
        let attn = LtxCausalAttention {
            to_qkv: LinearConfig::new(256, 768).with_bias(true).init(&device),
            to_out: LinearConfig::new(256, 256).with_bias(true).init(&device),
            num_heads: 4,
            head_dim: 64,
    ...
    #[test]
    fn test_ltx_tiny_forward() {
        let device = Default::default();
        let config = LtxVideoConfig::tiny();
